...
<a name="setup"></a>
## 3. Настройка проекта на использование GAS
Основные шаги по созданию проекта с использованием GAS:
1. Включите Plugin GameplayAbilitySystem в Unreal Editor
1. Отредактируйте `YourProjectName.Build.cs` добавив `"GameplayAbilities", "GameplayTags", "GameplayTasks"` к вашим `PrivateDependencyModuleNames`
1. Обновите файлы проекта Visual Studio
1. Начиная с версии 4.24, для использования [`TargetData`](#concepts-targeting-data) теперь обязательно нужно вызывать `UAbilitySystemGlobals::Get().InitGlobalData()`. В проекте-образце это делается в `UAssetManager::StartInitialLoading()`. Более подробную информацию смотрите в [`InitGlobalData()`](#concepts-asg-initglobaldata).

Это все, что вам нужно сделать, чтобы включить GAS. Отсюда добавьте [`ASC`](#concepts-asc) и [`AttributeSet`](#concepts-as) к своему `Character` или `PlayerState` и начинайте создавать [`GameplayAbilities`](#concepts-ga) and [`GameplayEffects`](#concepts-ge)!
...
`AbilitySystemComponent` (`ASC`) - это сердце GAS. Это `UActorComponent` ([`UAbilitySystemComponent`](https://docs.unrealengine.com/en-US/API/Plugins/GameplayAbilities/UAbilitySystemComponent/index.html)) который обрабатывает все взаимодействия с ASC. Любой `Actor` который хочет использовать [`GameplayAbilities`](#concepts-ga), иметь [`Attributes`](#concepts-a), или использовать [`GameplayEffects`](#concepts-ge) должен иметь `ASC` присоединенный к нему. Все эти объекты живут внутри ASC, управляются и реплицируются им (за исключением `Attributes` которые реплицируются [`AttributeSet`](#concepts-as)). Разработчики могут, но не обязаны создавать подклассы этих классов.

`Actor` к которому прикреплен `ASC` называется `OwnerActor`. Физическое представление `Actor` использующего `ASC` называется `AvatarActor`. `OwnerActor` и `AvatarActor` могут быть одним и тем же `Actor` как в случае простого ИИ-миньона в игре MOBA. Так же они могут быть разными `Actors` как в случае с персонажем, управляемым игроком в MOBA-игре, где `OwnerActor` это `PlayerState` и `AvatarActor` это предсталение персонажа в игре в виде класса `Character`. Большинство `Actors` будут иметь `ASC` на себе. Если ваш `Actor` будет перерождаться и ему требуется сохранение `Attributes` или `GameplayEffects` между перерождениями (как герою в MOBA), то идеальным местом для `ASC` будет `PlayerState`.

**Примечание:** Если ваш `ASC` находится в `PlayerState`, то вам необходимо увеличить `NetUpdateFrequency` для вашего `PlayerState`. По умолчанию в  `PlayerState` установлено очень низкое значение и это может вызвать задержки перед тем, как изменения таких вещей, как `Attributes` и `GameplayTags` произойдут на клиентах. Обязательно включите [`Adaptive Network Update Frequency`](https://docs.unrealengine.com/en-US/Gameplay/Networking/Actors/Properties/index.html#adaptivenetworkupdatefrequency), Fortnite использует ее.

Если `OwnerActor` и `AvatarActor` это разные `Actors`, то они оба должны реализовать интерфейс `IAbilitySystemInterface`. В этом интерфейсе есть одна функция, которую необходимо переопределить, `UAbilitySystemComponent* GetAbilitySystemComponent() const`, которая возвращает указатель на свой `ASC`. `ASCs` взаимодействуют друг с другом внутри системы, ища эту функцию интерфейса.

`ASC` хранит свои текущие активные `GameplayEffects` в `FActiveGameplayEffectsContainer ActiveGameplayEffects`.

`ASC`хранит предоставленные `Gameplay Abilities` в `FGameplayAbilitySpecContainer ActivatableAbilities`. Каждый раз, когда вы планируете выполнять итерации над `ActivatableAbilities.Items`, обязательно добавляйте `ABILITYLIST_SCOPE_LOCK();` выше вашего цикла, чтобы заблокировать список от изменения (из-за удаления способности). Каждый `ABILITYLIST_SCOPE_LOCK();` в области видимости увеличивает `AbilityScopeLockCount` а затем уменьшает, когда выпадает из области видимости. Не пытайтесь удалить способность внутри области действия `ABILITYLIST_SCOPE_LOCK();` (функции очистки способностей проверяют `AbilityScopeLockCount` внутренне, чтобы предотвратить удаление способностей, если список заблокирован).

<a name="concepts-asc-rm"></a>
### 4.1.1 Режимы репликации
В `ASC` определены три различных режима репликации для репликации `GameplayEffects`, `GameplayTags` и `GameplayCues` - `Full`, `Mixed` и `Minimal`. `Attributes` реплицируются по их `AttributeSet`.

| Режим репликации   | Когда использовать                        | Описание                                                                                                                        |
| ------------------ | ----------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------|
| `Full`             | Одиночный режим                           | Каждый `GameplayEffect` реплицируется каждому клиенту.                                                                          |
| `Mixed`            | Мультиплеер, игрок контроллирует `Actors` | `GameplayEffects` реплицируются только на клиента-владельца. Только `GameplayTags` и `GameplayCues` реплицируются всем.         |
| `Minimal`          | Мультиплеер, AI контроллирует `Actors`    | `GameplayEffects` никогда никому не передаются. Только `GameplayTags` и `GameplayCues` реплицируются всем.                      |

**Примечание:** `Mixed` replication mode expects the `OwnerActor's` `Owner` to be the `Controller`. `PlayerState's` `Owner` is the `Controller` by default but `Character's` is not. If using `Mixed` replication mode with the `OwnerActor` not the `PlayerState`, then you need to call `SetOwner()` on the `OwnerActor` with a valid `Controller`.
**Примечание:** Режим репликации `Mixed` ожидает, что `Owner` `OwnerActor` будет `Controller`. По умолчанию `Owner` `PlayerState` является `Controller`, а `Owner` `Character` - нет. Если используется режим репликации `Mixed` с `OwnerActor`, а не `PlayerState`, то необходимо вызвать `SetOwner()` для `OwnerActor` с действительным `Controller`.

Начиная с версии 4.24, `PossessedBy()` теперь устанавливает владельцем `Pawn` новый `Controller`.

**[⬆ Вернуться к началу](#table-of-contents)**

<a name="concepts-asc-setup"></a>
### 4.1.2 Настройка и Инициализация
Обычно `ASC` создаются в конструкторе `OwnerActor` и явно помечаются реплицируемыми. **Это должно быть сделано в C++**.
...
`ASC` должен быть инициализирован с его `OwnerActor` и `AvatarActor` как на сервере, так и на клиенте. Инициализировать необходимо после установки `Controller` в `Pawn` (после овладения). В однопользовательских играх нужно беспокоиться только о пути к серверу.

Для персонажей управляемых игроком, где `ASC` живет на `Pawn`, я обычно инициализирую на сервере в функции `PossessedBy()` для `Pawn` и инициализирую на клиенте в функции `PlayerController` `AcknowledgePossession()`.
...
Для персонажей, управляемых игроком, где `ASC` живет в `PlayerState`, я обычно инициализирую `Pawn` на сервер в функции `PossessedBy()` и на клиенте в функции `OnRep_PlayerState()`. Это гарантирует, что `PlayerState` существует на клиенте.
...
Если вы получите сообщение об ошибке `LogAbilitySystem: Warning: Can't activate LocalOnly or LocalPredicted ability %s when not local!`, то вы не инициализировали свой `ASC` на клиенте.

**[⬆ Вернуться к началу](#table-of-contents)**

<a name="concepts-gt"></a>
### 4.2 Gameplay Tags
[`FGameplayTags`](https://docs.unrealengine.com/en-US/API/Runtime/GameplayTags/FGameplayTag/index.html) — это имена в написанные в иерархическом формате `Parent.Child.Grandchild...`, зарегистрированные в `GameplayTagManager`. Эти теги невероятно полезны для классификации и описания состояния объекта. Например, если персонаж оглушен, мы можем присвоить ему `GameplayTag` со значением `State.Debuff.Stun` на время оглушения.

Вы обнаружите, что заменяете вещи, которые вы использовали для обработки логическими значениями или перечислениями, на `GameplayTags` и выполняете логику в зависимости от того, имеют ли объекты определенные `GameplayTags`.

Присваивая теги объекту, мы обычно добавляем их в его `ASC`, если он у него есть, чтобы GAS мог взаимодействовать с этими тэгами. `UAbilitySystemComponent` реализует `IGameplayTagAssetInterface`, предоставляя функции для доступа к собственным `GameplayTags`.

Несколько тегов `GameplayTags` могут храниться в `FGameplayTagContainer`. Предпочтительно использовать `GameplayTagContainer` вместо `TArray<FGameplayTag>`, так как `GameplayTagContainers` добавляет некоторую магию эффективности. Хотя теги являются стандартными именами `FNames`, их можно эффективно упаковать вместе в `FGameplayTagContainers` для репликации, если в настройках проекта включена `Fast Replication`. Для `Fast Replication` требуется, чтобы сервер и клиенты имели одинаковый список `GameplayTags`. Как правило, это не должно быть проблемой, поэтому вам следует включить эту опцию. `GameplayTagContainers` также может возвращать `TArray<FGameplayTag>` для итерации.

`GameplayTags`, хранящиеся в `FGameplayTagCountContainer`, имеют `TagMap`, в котором хранится количество экземпляров этого `GameplayTag`. `FGameplayTagCountContainer` может по-прежнему иметь в себе `GameplayTag`, но его `TagMapCount` равен нулю. Вы можете столкнуться с этим во время отладки, если в `ASC` все еще есть `GameplayTag`. Любая из функций `HasTag()` или `HasMatchingTag()` проверит `TagMapCount` и вернет `false`, если `GameplayTag` отсутствует или его `TagMapCount` равен нулю.

`GameplayTags` должны быть определены заранее в файле `DefaultGameplayTags.ini`. Редактор UE5 предоставляет интерфейс в Project Settings, позволяющий разработчикам управлять `GameplayTags` без необходимости вручную редактировать файл `DefaultGameplayTags.ini`. Редактор `GameplayTag` может создавать, переименовывать, искать ссылки и удалять `GameplayTags`.

![GameplayTag Editor в Project Settings](https://github.com/tranek/GASDocumentation/raw/master/Images/gameplaytageditor.png)

При поиске ссылок `GameplayTag` откроется знакомый график `Reference Viewer` в редакторе, показывающий все Assets, которые ссылаются на выбранный `GameplayTag`. Однако это не покажет никаких классов C++, которые ссылаются на `GameplayTag`.

Переименование `GameplayTags` создает перенаправление, так что Assets, все еще ссылающиеся на исходный `GameplayTag`, могут перенаправляться на новый `GameplayTag`. Я предпочитаю, если это возможно, вместо этого создать новый `GameplayTag`, вручную обновить все ссылки на новый `GameplayTag`, а затем удалить старый `GameplayTag`, чтобы избежать создания перенаправления.

В дополнение к `Fast Replication` редактор `GameplayTag` имеет возможность заполнять часто реплицируемые теги `GameplayTags` для их дальнейшей оптимизации.

`GameplayTags` реплицируются, если они добавляются из `GameplayEffect`. `ASC` позволяет вам добавлять `LooseGameplayTags`, которые не реплицируются и должны управляться вручную. В примере проекта используется `LooseGameplayTag` для `State.Dead`, чтобы клиенты-владельцы могли немедленно реагировать, когда их здоровье падает до нуля. Возрождение вручную сбрасывает `TagMapCount` обратно на ноль. Только вручную настраивайте `TagMapCount` при работе с `LooseGameplayTags`. Предпочтительнее использовать функции `UAbilitySystemComponent::AddLooseGameplayTag()` и `UAbilitySystemComponent::RemoveLooseGameplayTag()`, чем вручную настраивать `TagMapCount`.
...
```
Для расширенных манипуляций с `GameplayTag`, таких как получение родительских или дочерних `GameplayTags`, посмотрите на функции, предлагаемые `GameplayTagManager`. Чтобы получить доступ к `GameplayTagManager`, включите `GameplayTagManager.h` и вызовите его с помощью `UGameplayTagManager::Get().FunctionName`. `GameplayTagManager` фактически хранит `GameplayTags` как реляционные узлы (родительский, дочерний и т. д.) для более быстрой обработки, чем постоянные манипуляции со строками и сравнения.

`GameplayTags` и `GameplayTagContainers` могут иметь необязательный спецификатор `UPROPERTY` `Meta = (Categories = "GameplayCue")`, который фильтрует теги в Blueprint для отображения только `GameplayTags`, у которых есть родительский тег `GameplayCue`. Это полезно, когда вы знаете, что переменная `GameplayTag` или `GameplayTagContainer` должна использоваться только для `GameplayCues`.

В качестве альтернативы существует отдельная структура под названием `FGameplayCueTag`, которая инкапсулирует `FGameplayTag`, а также автоматически фильтрует `GameplayTags` в Blueprint, чтобы отображались только те теги, у которых есть родительский тег `GameplayCue`.

Если вы хотите отфильтровать параметр `GameplayTag` в функции, используйте спецификатор `UFUNCTION` `Meta = (GameplayTagFilter = "GameplayCue")`. Параметры `GameplayTagContainer` в функциях фильтровать нельзя. Если вы хотите отредактировать свой движок, чтобы разрешить это, посмотрите, как `SGameplayTagGraphPin::ParseDefaultValueData()` из `Engine\Plugins\Editor\GameplayTagsEditor\Source\GameplayTagsEditor\Private\SGameplayTagGraphPin.cpp` вызывает `FilterString = UGameplayTagsManager::Get().GetCategoriesMetaFromField(PinStructType);` и передает `FilterString` в `SGameplayTagWidget` в `SGameplayTagGraphPin::GetListContent()`. Версия этих функций `GameplayTagContainer` в `Engine\Plugins\Editor\GameplayTagsEditor\Source\GameplayTagsEditor\Private\SGameplayTagContainerGraphPin.cpp` не проверяет свойства метаполя и проходит через фильтр.

Пример проекта широко использует `GameplayTags`.

**[⬆ Вернуться к началу](#table-of-contents)**

<a name="concepts-gt-change"></a>
### 4.2.1 Реакция на изменения in Gameplay Tags
`ASC` предоставляет делегат для добавления или удаления тегов `GameplayTags`. Он принимает `EGameplayTagEventType`, который может запускаться только при добавлении/удалении `GameplayTag` или при любых изменениях `GameplayTag` в `TagMapCount`.

```c++
AbilitySystemComponent->RegisterGameplayTagEvent(FGameplayTag::RequestGameplayTag(FName("State.Debuff.Stun")), EGameplayTagEventType::NewOrRemoved).AddUObject(this, &AGDPlayerState::StunTagChanged);
```

Функция обратного вызова имеет параметр для `GameplayTag` и нового `TagCount`.
```c++
...
`Attribute` содержит два значения - `BaseValue` и `CurrentValue`. `BaseValue` это постоянное значение атрибута, а `CurrentValue` это `BaseValue` плюс временные изменения от `GameplayEffects`. Например, у вашего `Character` может быть `Attribute` скорости передвижения с `BaseValue` установленным на 600 ед/с. Поскольку `GameplayEffects` изменяющих скорость передвижения пока нет, `CurrentValue` также равно 600 ед/с. Если `Character` получит временный бафф скорости передвижения, `BaseValue` останется прежним - 600 ед/с, а `CurrentValue` станет 600 + 50, что в сумме составит 650 ед/с. Когда действие баффа скорости передвижения закончится, `CurrentValue` вернется обратно к `BaseValue` на 600 ед/с.

Часто новички в GAS путают `BaseValue` с максимальным значением `Attribute` и пытаются считать его таковым. Это неверный подход.  Максимальные значения для `Attributes` которые могут изменяться или на которые есть ссылки в способностях или пользовательском интерфейсе, должны рассматриваться как отдельные `Attributes`. Для жестко закодированных максимальных и минимальных значений существует способ определить `DataTable` с `FAttributeMetaData`, который может устанавливать максимальные и минимальные значения, но в комментарии Epic над структурой говорится, что это "работа еще ведется". Смотрите `AttributeSet.h` для получения дополнительной информации. Чтобы избежать путаницы, я рекомендую сделать максимальные значения, на которые можно ссылаться в способностях или UI, отдельными `Attributes`, а жестко закодированные максимальные и минимальные значения, которые используются только для зажатия значений в рамках `Attributes`, определить как жестко закодированные плавающие значения в `AttributeSet`. Зажатие атрибутов обсуждается в [PreAttributeChange()](#concepts-as-preattributechange) для изменений в `CurrentValue` и [PostGameplayEffectExecute()](#concepts-as-postgameplayeffectexecute) для изменений в `BaseValue` от `GameplayEffects`.

Постоянные изменения `BaseValue` происходят от `Instant` (мгновенных) `GameplayEffects`, в то время как `Duration` (длительные) и `Infinite` (бесконечные) `GameplayEffects` изменяют `CurrentValue`. Периодические `GameplayEffects` рассматриваются как мгновенные `GameplayEffects` и изменяют `BaseValue`.
...
`GameplayEffects` содержат несколько контейнеров [`GameplayTagContainers`](#concepts-gt). Дизайнеры будут редактировать `Added` (Добавляя) и `Removed` (Удаляя) `GameplayTagContainers` для каждой категории, и результат будет отображаться в `Combined` `GameplayTagContainer` при компиляции. `Added` (Добавленные) теги — это новые теги, добавляемые этим `GameplayEffect`, которых ранее не было у его родителей. `Removed` (Удаленные) теги — это теги, которые есть у родительских классов, но отсутствуют у этого подкласса.

| Категория                         | Описание                                                                                                                                                                                                                                                                                                                                                                                                                        |
|-----------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Gameplay Effect Asset Tags        | Теги, которые имеет `GameplayEffect`. Они не выполняют никаких функций сами по себе и служат только для описания `GameplayEffect`.                                                                                                                                                                                                                                                                                              |
| Granted Tags                      | Теги, которые живут в `GameplayEffect`, но также передаются `ASC`, к которому применяется `GameplayEffect`. Они удаляются из `ASC` при удалении `GameplayEffect`. Это работает только для `GameplayEffects` с типом продолжительности `Duration` and `Infinite`.                                                                                                                                                                |
| Ongoing Tag Requirements          | После применения эти теги определяют, включен или отключен `GameplayEffect`. `GameplayEffect` может быть отключен, но все равно применяться. Если `GameplayEffect` отключен из-за невыполнения текущих требований к тегу, но затем требования соблюдены, `GameplayEffect` снова включится и повторно применит свои модификаторы. Это работает только для `GameplayEffects` с типом продолжительности `Duration` and `Infinite`. |
| Application Tag Requirements      | Теги в `Target` (Цели), которые определяют, можно ли применить текущий `GameplayEffect` к `Target` (Цели). Если эти требования не выполняются, `GameplayEffect` не применяется.                                                                                                                                                                                                                                                 |
| Remove Gameplay Effects with Tags | `GameplayEffects` на `Target` (Цели), которые имеют любой из этих тегов в своих `Asset Tags` или `Granted Tags`, будут удалены из `Target` (Цели) при успешном применении этого `GameplayEffect`.                                                                                                                                                                                                                               |
...
#### 4.6.9 Ability Tags
`GameplayAbilities` поставляются с `GameplayTagContainers` со встроенной логикой. Ни один из этих `GameplayTags` не реплицируется.

| `GameplayTag Container`     | Описание                                                                                                                                                                                                                         |
| --------------------------- |----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `Ability Tags`              | `GameplayTags`, которыми владеет `GameplayAbility`. Это всего лишь `GameplayTags` для описания `GameplayAbility`.                                                                                                                |
| `Cancel Abilities with Tag` | Другие `GameplayAbilities`, у которых есть перечисленные `GameplayTags` в `Ability Tags`, будут отменены при активации этой `GameplayAbility`.                                                                                   |
| `Block Abilities with Tag`  | Другие `GameplayAbilities`, у которых есть перечисленные `GameplayTags` в `Ability Tags`, блокируются от активации, пока эта `GameplayAbility` активна.                                                                          |
| `Activation Owned Tags`     | Данные `GameplayTags` еакладываются на владельца `GameplayAbility`, пока текущая `GameplayAbility` активна. Помните, что они не реплицируются.                                                                                   |
| `Activation Required Tags`  | Текущая `GameplayAbility` может быть активирована только в том случае, если у владельца есть все перечисленные здесь теги `GameplayTags`.                                                                                        |
| `Activation Blocked Tags`   | Текущая `GameplayAbility` не может быть активирована, если у владельца есть **любой** из этих `GameplayTags`.                                                                                                                    |
| `Source Required Tags`      | Текущая `GameplayAbility` может быть активирована только в том случае, если `Source` имеет **все** эти `GameplayTags`. `Source` `GameplayTags` устанавливается только в том случае, если `GameplayAbility` запускается событием. |
| `Source Blocked Tags`       | Текущая `GameplayAbility` не может быть активирована, если `Source` имеет **любой** из этих `GameplayTags`. `Source` `GameplayTags` устанавливается только в том случае, если `GameplayAbility` запускается событием.            |
| `Target Required Tags`      | Текущая `GameplayAbility` может быть активирована, только если у `Target` есть **все** эти `GameplayTags`. `Target` `GameplayTags` устанавливаются только в том случае, если `GameplayAbility` запускается событием.             |
| `Target Blocked Tags`       | Текущая `GameplayAbility` не может быть активирована, если `Target` имеет **любой** из этих `GameplayTags`. `Target` `GameplayTags` устанавливаются только в том случае, если `GameplayAbility` запускается событием.            |
